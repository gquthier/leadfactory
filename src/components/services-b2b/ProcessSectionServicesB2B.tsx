...
const steps = [
  {
    ...
    description: "Vous nous expliquez vos services, votre cible idéale, le budget moyen de vos clients.",
    icon: BarChart3,
  },
  {
    number: "02", 
    title: "ON CRÉE TOUT POUR VOUS",
    description: "Vidéos, visuels, publicités ciblées sur les entreprises qui ont besoin de vos services.",
    icon: Rocket,
  },
  {
    number: "03",
    title: "VOUS RECEVEZ DES DEMANDES",
    description: "Des entreprises qualifiées vous contactent. Vous présentez, vous signez.",
    icon: Trophy,
  },
];

export const ProcessSectionServicesB2B = () => {
  return (
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="text-center mb-16">
          <h2 className="responsive-headline mb-8">
            3{" "}
            <span className="italic text-[#3B82F6] transform -skew-x-12 inline-block">ÉTAPES.</span>
            <br />
          ...
          {steps.map((step, index) => (
            <div key={index} className="brutalist-primary-card relative">
              ...
              <div className="space-y-6">
                ...
                <h3 className="text-xl font-black text-white uppercase tracking-wider">{step.title}</h3>
                <p className="text-lg font-bold text-white leading-relaxed">{step.description}</p>
                <div className="mt-6 h-20 bg-white border-3 border-black p-4 flex items-end justify-center gap-2">
                  {[40, 60, 80, 95, 100].map((height, i) => (
                    <div key={i} className="bg-[#FFC4EB] border border-black flex-1 max-w-3" style={{ height: `${height}%` }}></div>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="mt-16 brutalist-card bg-[#FDE047] text-center max-w-4xl mx-auto">
          <h3 className="text-2xl font-black uppercase mb-4">PENDANT CE TEMPS, VOUS LIVREZ VOS PROJETS.</h3>
          <p className="text-xl font-bold text-black"><span className="italic">Nous, on remplit votre pipeline de prospects.</span> 💼✨</p>
        </div>
      </div>
    </section>
  );
